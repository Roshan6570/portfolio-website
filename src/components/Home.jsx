...
const NAME    = 'Roshan Patil'
const ROLE    = 'Full Stack Developer'
const TAGLINE = 'I build beautiful, performant web applications with modern technologies. Passionate about clean code and great user experiences.'
const GITHUB  = 'https://github.com/Roshan6570'
const LINKEDIN= 'https://www.linkedin.com/in/roshan-patil-16350b320/'
const PROFILE_IMG = '/profile.png'   // put image in /public and uncomment
// ─────────────────────────────────────────────────────────────────────────
...
export default function Home() {
  return (
    <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-12">
      ...
      >
        {/* If you have a profile image, replace the div below with:
            <img src={PROFILE_IMG} alt={NAME} className="w-60 h-60 sm:w-72 sm:h-72 rounded-full object-cover ring-4 ring-indigo-500/30 shadow-2xl shadow-indigo-900/50" />
        */}
        <div className="relative w-60 h-60 sm:w-72 sm:h-72">
          <div className="absolute inset-0 rounded-full bg-indigo-600/20 animate-ping" style={{ animationDuration: '3s' }} />
          <div className="relative w-full h-full rounded-full bg-gradient-to-br from-indigo-500 via-purple-600 to-pink-500
             flex items-center justify-center text-7xl shadow-2xl shadow-indigo-900/50 ring-4 ring-indigo-500/30">
            
             <img src="/Roshan.jpeg" alt="Profile"
                      className='w-60 h-70 sm:w-72 sm:h-72 rounded-full object-cover
                       ring-4 ring-indigo-500/30 shadow-2xl shadow-indigo-900/50'
              />
          </div> 
        </div>
      </motion.div>
